...
This library automatically switches between two messaging backends depending on the tab's container state:
1. **BroadcastChannel**: Used for the default container tabs (`firefox-default`). Fast and secure.
2. **Hash Messaging**: Used for private or non-default container tabs where `BroadcastChannel` cannot bridge the communication. It passes messages by chunking and encoding them through the URL fragment identifier (`location.hash`).
...
**BroadcastChannel backend (Default container tabs):**
...
**Hash backend (Private or Non-default container tabs):**
...
CrossContextMessagingBG.onMessage(async (message, sender) => {
  if (message.type === 'GREETING') {
    // sender.tab is automatically populated from the mapped clientId or directly from hash messaging
