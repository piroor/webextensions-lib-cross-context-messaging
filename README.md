...
2. **Hash Messaging**: Used for non-default container tabs where `BroadcastChannel` cannot bridge the communication. It passes messages by chunking and encoding them through the URL fragment identifier (`location.hash`).
...
**Hash backend (Non-default container tabs):**
