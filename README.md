...
Important note: **you cannot send messages between tabs with different containers**. Only the default container tabs can communicate with the background script via this channel. You should use [Hash Messaging](https://github.com/piroor/webextensions-lib-hash-messaging) if you hope to communicate with non-default container tabs.
...
CrossContextMessagingBG.onMessage(async (message, sender) => {
  if (message.type === 'GREETING') {
    // sender.tab is automatically populated from the mapped clientId
...
/*
const response = await CrossContextMessagingBG.sendMessage(sender.tab.id, {
  type: 'INIT_DATA',
  payload: { userId: 123 }
});
